...
function Lightbox({ images, startIndex, onClose, accentColor, type }: LightboxProps) {
    ...
    console.log(images[current]); // Debugging line to check images passed to Lightbox
        ...
        >
            ...
            <div className="flex items-center justify-between px-6 py-4 z-10">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {/* Download */}
                    <button
                        onClick={() => downloadKazepiFile(images[current].id, type === 'kazepi' ? 'kazepi' : type === 'uzepi' ? 'uzepi' : type === 'passport' ? 'passport' : 'queue')}
                        className="p-2 rounded-lg bg-white/10 hover:bg-white/20 text-white transition-colors"
                        title="Yuklab olish"
                    >
                        <Download size={18} />
            ...
            <div className="flex-1 flex items-center justify-center overflow-hidden relative px-16">
                ...
                <AnimatePresence mode="wait">
                    ...
                    {
                        images[current].mime_type === 'application/pdf' ? <File size={50} color="#fff" />
...
export default function ImageViewerModal({ open, onClose, images, imgType, type }: ImageViewerModalProps) {
    ...
    // console.log("Active category:", activeCat); // Debugging line to check active category
                                                    ...
                                                    <motion.div
                                                        ...
                                                        onClick={() => {
                                                            ...
                                                            if (img.mime_type === 'application/pdf') {
                                                                ...
                                                                window.open(`https://mobile-test.izisol.uz/storage/${img.path}`, '_blank')

                                                                console.log(img)

                                                            } else {
                                                                openLightbox(activeCat?.images, i, activeCat?.accent)
                                                            }
                                                        }}
                                                    >
                                                        ...
                                                        <div
                                                            ...
                                                            style={{
                                                                background: img.status === 'approved' ? "linear-gradient(to top, rgba(3, 77, 22, 0.8), transparent)"  : img.status === 'pending' ?  "linear-gradient(to top, rgba(108, 91, 4, 0.8), transparent)" :  "linear-gradient(to top, rgba(108, 4, 4, 0.8), transparent)",
                                                            }}
                                                        >
                                                            <div className="flex items-start justify-between">
                                                                <p className="text-white text-[11px] font-medium truncate">{img.type}</p>
                                                                <p className="text-white text-[11px] font-medium truncate">{img.status}</p>
                                                            </div>
                                                        </div>

                                                        {/* Download btn on hover */}
                                                        <button
                                                            onClick={() => downloadKazepiFile(img.id, type === 'kazepi' ? 'kazepi' : 'queue')}
