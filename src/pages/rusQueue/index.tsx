...
type QueueStatistics =
    "russia_queue_pending"
    | "russia_queue_approved"
    | "russia_queue_rejected"
    | "russia_queue_payment_uploaded"
    | "russia_queue_payment_approved"
...
const statisticsConfig: Record<QueueStatistics, { label: string; subClass: string }> = {
    russia_queue_pending: { label: "Kutilmoqda", subClass: "text-blue-500" },
    russia_queue_approved: { label: "Tasdiqlandi", subClass: "text-emerald-500" },
    russia_queue_payment_uploaded: { label: "To‘lov yuklandi", subClass: "text-yellow-500" },
    russia_queue_payment_approved: { label: "Hujjat yuklanmagan", subClass: "text-orange-500" },
    russia_queue_rejected: { label: "Rad etildi", subClass: "text-red-500" },
};
...
export default function RusQueue() {
    ...
    const getQueue = async (page: number, searchValue = search) => {
        try {
            ...
            const res = await api.get(`/admin/russia-queue${status === 'pending' ? '' : status === 'success' ? '/approved-history' : '/rejected-history'}`, {
                params: {
                    page,
    ...
    const getStatistics = async () => {
        try {
            const res = await api.get(`/admin/russia-queue/statistics`);
