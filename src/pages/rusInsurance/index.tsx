...
const statisticsConfig: Record<QueueStatistics, { label: string; subClass: string }> = {
    insurance_pending: { label: "Kutilmoqda", subClass: "text-blue-500" },
    insurance_approved: { label: "Tasdiqlandi", subClass: "text-emerald-500" },
    insurance_payment_uploaded: { label: "To‘lov yuklandi", subClass: "text-yellow-500" },
    insurance_payment_approved: { label: "Hujjat yuklanmagan", subClass: "text-orange-500" },
    insurance_rejected: { label: "Rad etildi", subClass: "text-red-500" },
};
...
export default function RusKazInsurance() {
    ...
    const handleSync = () => {
        setSyncing(true);
        void getRusInsurance(currentPage);
    ...
    const getRusInsurance = async (page: number) => {
        try {
            ...
            const res = await api.get(`/admin/insurance${status === 'pending' ? '' : status === 'success' ? '/approved-history' : '/rejected-history'}`, {
                params: {
                    page,
    ...
    const getStatistics = async () => {
        ...
        try {
            const res = await api.get(`/admin/insurance/statistics`);
            ...
            <div className="mx-auto">
                ...
                {meta && data && (
                    <QueueTable
                        ...
                        refetch={() => getRusInsurance(currentPage)}
                        type={'insurance'}
